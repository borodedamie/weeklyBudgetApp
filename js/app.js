...
class HTML{
    ...
    trackBudget(amount) {
         const budgetLeftNaira = budget.substractFromBudget(amount);
         budgetLeft.innerHTML = `${budgetLeftNaira}`;

         // Check when 25% is left
         if( (budget.budget / 4 ) > budgetLeftNaira ){
              budgetLeft.parentElement.parentElement.classList.remove('alert-success', 'alert-warning');
              budgetLeft.parentElement.parentElement.classList.add('alert-danger');

         } else if( (budget.budget / 2 ) > budgetLeftNaira  ) {
